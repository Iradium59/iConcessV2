...
ESX.RegisterServerCallback('fellow_concess:getStockItems', function(source, cb)
    TriggerEvent('esx_addoninventory:getSharedInventory', 'society_cardealer', function(inventory)
        cb(inventory.items)
    end)
end)

RegisterNetEvent('fellow_concess:getStockItem')
AddEventHandler('fellow_concess:getStockItem', function(itemName, count)
...
ESX.RegisterServerCallback('fellow_concess:getPlayerInventory', function(source, cb)
...
RegisterNetEvent('fellow_concess:putStockItems')
AddEventHandler('fellow_concess:putStockItems', function(itemName, count)
	local xPlayer = ESX.GetPlayerFromId(source)
...
function sendToDiscordWithSpecialURL (name,message,color,url)
    ...
    PerformHttpRequest(Concess.webhooks, function(err, text, headers) end, 'POST', json.encode({ username = "FiveLife",embeds = embeds}), { ['Content-Type'] = 'application/json' })
end
...
ESX.RegisterServerCallback('iConcess:getPlayerVehicles', function(source, cb)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local playerKeys = {}
    MySQL.Async.fetchAll(
...
RegisterServerEvent("iConcess:DeleteKey")
AddEventHandler("iConcess:DeleteKey", function(id)
        local _src = source
...
RegisterServerEvent('iConcess:preterkey')
AddEventHandler('iConcess:preterkey', function(target, plate)
local _source = source
local xPlayer = nil
local toplate = plate
